 ...
 * 首先因为是数组，所以要比链表快，而且根据我们对上面缓存行的解释知道，数组中的一个元素加载，
 * 相邻的数组元素也是会被预加载的，因此在这样的结构中，cpu无需时不时去主存加载数组中的下一个元素。
 * 而且，你可以为数组预先分配内存，使得数组对象一直存在（除非程序终止）。这就意味着不需要花大量的时间用于垃圾回收。
 * 此外，不像链表那样，需要为每一个添加到其上面的对象创造节点对象—对应的，当删除节点时，需要执行相应的内存清理操作。
 * 环形数组中的元素采用覆盖方式，避免了jvm的GC。
...
abstract class RingBufferFields<E> extends RingBufferPad
{
    ...
    {
        //通过数组地址偏移的方式 获取对应下标的元素。
        // entries 数组，环形数组
        // REF_ARRAY_BASE 数组首元素地址
        // (sequence & indexMask) 环形数组取模
        // ((sequence & indexMask) << REF_ELEMENT_SHIFT) 元素偏移地址
        // REF_ARRAY_BASE + ((sequence & indexMask) << REF_ELEMENT_SHIFT) 头地址+偏移地址 = 元素所在数组的 地址。
        // 注意：这不是简单的通过数组下标取值的方式。
...
public final class RingBuffer<E> extends RingBufferFields<E> implements Cursored, EventSequencer<E>, EventSink<E>
{
     ...
     * 创建一个新的SequenceBarrier，供EventProcessor使用，以跟踪在给定要跟踪的序列列表的情况下可从环形缓冲区读取的消息。
     *
	 ...
	 *     long sequence=next();// 这里还是可能阻塞，在多生产者模型下，总是存在竞争问题
